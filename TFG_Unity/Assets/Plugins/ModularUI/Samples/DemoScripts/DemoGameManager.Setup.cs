            ...
            if (targetShader != null)
            {
                ...
                foreach (var r in renderers)
                {
                    ...
                    if (needsFix)
                    {
                        ...
                        try
                        {
                            if (r.sharedMaterial.HasProperty("_Color")) oldColor = r.sharedMaterial.color;
                            else if (r.sharedMaterial.HasProperty("_BaseColor")) oldColor = r.sharedMaterial.GetColor("_BaseColor");
                            if (r.sharedMaterial.HasProperty("_MainTex")) oldMainTex = r.sharedMaterial.mainTexture;
                            else if (r.sharedMaterial.HasProperty("_BaseMap")) oldMainTex = r.sharedMaterial.GetTexture("_BaseMap");
                        } catch {}

                        Material newMat = new Material(targetShader);
                        newMat.name = r.sharedMaterial.name + "_Fixed";
                        if (newMat.HasProperty("_Color")) newMat.color = oldColor;
                        else if (newMat.HasProperty("_BaseColor")) newMat.SetColor("_BaseColor", oldColor);
                        if (oldMainTex != null)
                        {
                            if (newMat.HasProperty("_MainTex")) newMat.mainTexture = oldMainTex;
                            else if (newMat.HasProperty("_BaseMap")) newMat.SetTexture("_BaseMap", oldMainTex);
                        }

                        if (Application.isPlaying) r.material = newMat;
                        else
                        {
